...
function xmldb_learningapp_upgrade($oldversion) {
    ...
    if ($oldversion < 2026090501) {
        // Snapshots generated by 1.4.0 embedded images/CSS/JS correctly but
        // still left the nested exercise's runtime data request
        // (learningapps.org/data?id=XXXXX, loaded via a synchronous XHR)
        // unhandled, so the exercise itself never got past its loading
        // spinner. Discard existing snapshots again so they regenerate with
        // the new XMLHttpRequest shim (see storage_manager::inject_data_shim()).
        $DB->delete_records('files', [
            'component' => 'mod_learningapp',
            'filearea'  => 'localstorage',
        ]);
        ...
        upgrade_mod_savepoint(true, 2026090501, 'learningapp');
    }
